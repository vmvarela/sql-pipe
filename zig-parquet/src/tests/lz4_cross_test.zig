//! Cross-implementation LZ4 tests
//!
//! Validates interoperability between C libLZ4 and pure Zig LZ4 implementations.
//! Cross-impl tests run when both are compiled in: -Dcodecs=lz4,zig-lz4
...
const zig_lz4 = @import("../core/compress/lz4.zig");
...
fn zigRoundTrip(allocator: std.mem.Allocator, data: []const u8) !void {
    if (!build_options.enable_zig_lz4) return;
    const compressed = try zig_lz4.compress(allocator, data);
    defer allocator.free(compressed);

    const decompressed = try zig_lz4.decompress(allocator, compressed, data.len);
...
fn crossRoundTrip(allocator: std.mem.Allocator, data: []const u8) !void {
    if (!build_options.enable_zig_lz4) return;
    const zig_compressed = try zig_lz4.compress(allocator, data);
    defer allocator.free(zig_compressed);

    if (both_enabled) {
        // Zig compress → C decompress
        const zc = try c_lz4.decompress(allocator, zig_compressed, data.len);
        defer allocator.free(zc);
        try std.testing.expectEqualSlices(u8, data, zc);

        // C compress → Zig decompress
        const c_compressed = try c_lz4.compress(allocator, data);
        defer allocator.free(c_compressed);
        const cz = try zig_lz4.decompress(allocator, c_compressed, data.len);
        defer allocator.free(cz);
        try std.testing.expectEqualSlices(u8, data, cz);
    }

    // Zig compress → Zig decompress
    const decompressed = try zig_lz4.decompress(allocator, zig_compressed, data.len);
    defer allocator.free(decompressed);
    try std.testing.expectEqualSlices(u8, data, decompressed);
}

// =========================================================================
// Cross-implementation tests (require both C and Zig LZ4)
// =========================================================================
...
test "cross-impl: C compress, Zig decompress" {
    ...
    const original = "Hello, World! This is a cross-implementation LZ4 test." ** 20;

    const compressed = try c_lz4.compress(allocator, original);
    defer allocator.free(compressed);

    const decompressed = try zig_lz4.decompress(allocator, compressed, original.len);
    defer allocator.free(decompressed);
...
test "cross-impl: Zig compress, C decompress" {
    ...
    const compressed = try zig_lz4.compress(allocator, original);
    defer allocator.free(compressed);

    const decompressed = try c_lz4.decompress(allocator, compressed, original.len);
    defer allocator.free(decompressed);
...
test "cross-impl: bidirectional round-trip (all 4 combinations)" {
    ...
    const c_compressed = try c_lz4.compress(allocator, original);
    defer allocator.free(c_compressed);
    const zig_compressed = try zig_lz4.compress(allocator, original);
    defer allocator.free(zig_compressed);

    const cc = try c_lz4.decompress(allocator, c_compressed, original.len);
    defer allocator.free(cc);
    try std.testing.expectEqualStrings(original, cc);

    const cz = try zig_lz4.decompress(allocator, c_compressed, original.len);
    defer allocator.free(cz);
    try std.testing.expectEqualStrings(original, cz);

    const zc = try c_lz4.decompress(allocator, zig_compressed, original.len);
    defer allocator.free(zc);
    try std.testing.expectEqualStrings(original, zc);

    const zz = try zig_lz4.decompress(allocator, zig_compressed, original.len);
...
test "cross-impl: empty data" {
    ...
    const c_compressed = try c_lz4.compress(allocator, "");
    defer allocator.free(c_compressed);
    const cz = try zig_lz4.decompress(allocator, c_compressed, 0);
    defer allocator.free(cz);
    try std.testing.expectEqual(@as(usize, 0), cz.len);

    const zig_compressed = try zig_lz4.compress(allocator, "");
    defer allocator.free(zig_compressed);
    const zc = try c_lz4.decompress(allocator, zig_compressed, 0);
    defer allocator.free(zc);
...
test "cross-impl: varied patterns" {
    ...
    for (patterns) |original| {
        const zig_compressed = try zig_lz4.compress(allocator, original);
        defer allocator.free(zig_compressed);
        const decompressed = try c_lz4.decompress(allocator, zig_compressed, original.len);
        defer allocator.free(decompressed);
        try std.testing.expectEqualStrings(original, decompressed);
    }
}
...
test "cross-impl: random incompressible data" {
    ...
    const cz = try zig_lz4.decompress(allocator, c_compressed, size);
    defer allocator.free(cz);
    try std.testing.expectEqualSlices(u8, data, cz);

    const zc = try c_lz4.decompress(allocator, zig_compressed, size);
    defer allocator.free(zc);
    try std.testing.expectEqualSlices(u8, data, zc);
}
